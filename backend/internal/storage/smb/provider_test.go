...
func TestNewSMBProvider(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check defaults
			if tt.config.Version == "" && provider.config.Version != "3.0" {
			...
			if tt.config.FileMode == "" && provider.config.FileMode != "0644" {
				t.Errorf("Expected default file mode 0644, got %s", provider.config.FileMode)
			}

			if tt.config.DirMode == "" && provider.config.DirMode != "0755" {
...
func TestBuildMountArgs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			optionsStr := args[oIndex+1]

			// Verify version is in options
			expectedVersion := "vers=" + tt.config.Version
			...
			if tt.config.Username != "" {
				expectedUsername := "username=" + tt.config.Username
				if !containsSMBOption(optionsStr, expectedUsername) {
					t.Errorf("Expected options to contain '%s'", expectedUsername)
				}
			}

			// Verify domain if provided
			if tt.config.Domain != "" {
				expectedDomain := "domain=" + tt.config.Domain
				if !containsSMBOption(optionsStr, expectedDomain) {
					t.Errorf("Expected options to contain '%s'", expectedDomain)
				}
			}

			// Verify Kerberos if enabled
			if tt.config.UseKerberos {
				if !containsSMBOption(optionsStr, "sec=krb5") {
					t.Error("Expected options to contain 'sec=krb5'")
				}
			}

			// Verify DirectIO if enabled
			if tt.config.DirectIO {
				if !containsSMBOption(optionsStr, "directio") {
					t.Error("Expected options to contain 'directio'")
				}
			}

			// Verify file and dir modes
			if tt.config.FileMode != "" {
				expectedFileMode := "file_mode=" + tt.config.FileMode
				if !containsSMBOption(optionsStr, expectedFileMode) {
					t.Errorf("Expected options to contain '%s'", expectedFileMode)
				}
			}

			if tt.config.DirMode != "" {
				expectedDirMode := "dir_mode=" + tt.config.DirMode
				if !containsSMBOption(optionsStr, expectedDirMode) {
					t.Errorf("Expected options to contain '%s'", expectedDirMode)
				}
			}

			// Check that SMB path and mount point are at the end
			if args[len(args)-2] != tt.smbPath {
				t.Errorf("Expected SMB path %s, got %s", tt.smbPath, args[len(args)-2])
			}
